...
> 一个基于 **React**, **Three.js (R3F)** 和 **AI 手势识别** 的高保真 3D 圣诞树 Web 应用，集成背景音乐和音乐播放控制面板。
...
### 3. 添加圣诞节音乐
   - 将你的圣诞节音乐文件放入 `public/audio` 目录
   - 将文件重命名为 `christmas-music.mp3`
   - 确保文件格式为 MP3
...
- 页面加载完成后，浏览器允许的情况下音乐会自动开始播放
- 使用音乐控制面板可以播放/暂停、调节音量、拖动进度条跳转播放位置
- 音乐会持续循环播放
- 如果浏览器阻止自动播放，请点击播放按钮手动启动音乐
